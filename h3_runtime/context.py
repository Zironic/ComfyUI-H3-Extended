...
the block stack starts.

Features may opt into ``forbid_device_sync``. In that mode the runtime never
reads CUDA tensor values on the host: sampler evaluations are counted from the
explicit OUTER_SAMPLE boundary instead of materializing sigma/schedule values.
"""
...
class H3RuntimeSession:
    ...
    def __init__(
        ...
        *,
        ...
        forbid_device_sync=False,
    ):
        self.strict_layout = bool(strict_layout)
        self.listeners = list(listeners)
        self.forbid_device_sync = bool(forbid_device_sync)
        self.request_id = -1
        self.layout_signature = None
        self._last_step = {}
        self._last_sigma = {}
        self._direction = {}
        self._host_forward_index = {}
    ...
    def enable_no_device_sync(self):
        """Forbid host reads of CUDA tensors for this shared runtime session."""
        self.forbid_device_sync = True
        return self

    def _branch(self, transformer_options):
        value = transformer_options.get("cond_or_uncond")
        if value is None:
            return (0,)
        if torch.is_tensor(value):
            flat = value.detach().flatten()
            if self.forbid_device_sync and flat.device.type == "cuda":
                # Shape metadata is host-visible without synchronizing. Combined
                # CFG commonly carries two entries; represent those structurally.
                # A single CUDA value cannot be inspected without violating the
                # no-sync contract, so treat it as the sole branch.
                count = int(flat.numel())
                return tuple(range(count)) if count > 1 else (0,)
            value = flat.tolist()
        if isinstance(value, (list, tuple)):
            return tuple(int(x) for x in value) or (0,)
        return (int(value),)

    def _scalar_sigma(self, timestep, transformer_options):
        if self.forbid_device_sync:
            # Sigma is diagnostic metadata only for no-sync clients. Do not
            # materialize a CUDA scalar merely to label an evaluation.
            return float("nan")
    ...
    @staticmethod
    def _step_index_from_sigma(sigma, transformer_options):
        schedule = transformer_options.get("sample_sigmas")
    ...
    def _host_step_index(self, branch, transformer_options):
        """Count model evaluations without inspecting any device tensor values."""
        total = self._schedule_length(transformer_options)
        index = int(self._host_forward_index.get(branch, 0))
        self._host_forward_index[branch] = index + 1
        if total:
            index = min(index, max(0, total - 1))
        return index, total
    ...
    def _reset_request(self, layout_signature):
        ...
        self._host_forward_index.clear()
    ...
    def observe(self, x, timestep, context, transformer_options, payload=None):
        ...
        if self.forbid_device_sync:
            if explicit:
                step_index, total_steps = self._host_step_index(
                    branch,
                    transformer_options,
                )
            else:
                # Direct model calls have no reliable no-sync step boundary.
                # Publish unknown step metadata; clients must fall back dense.
                step_index = -1
                total_steps = self._schedule_length(transformer_options)
        else:
            step_index, total_steps = self._step_index_from_sigma(
                sigma,
                transformer_options,
            )
        ...
        with self._lock:
            ...
            if explicit and self._explicit_layout_pending:
                ...
                new_request = layout_signature != self.layout_signature
                if not new_request and not self.forbid_device_sync:
...
def make_apply_model_wrapper(session):
    ...
    def wrapper(executor, *args, **kwargs):
        ...
        if (
            not session.forbid_device_sync
            and transformer_options.get("sigmas") is None
...
def install_runtime_wrapper(model_patcher, session=None):
    ...
    logging.info(
        "%s installed explicit sampler and diffusion request context (no_device_sync=%s)",
        LOG_PREFIX,
        session.forbid_device_sync,
    )
    return session
