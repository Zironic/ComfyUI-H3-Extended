...
In particular, production execution/reporting must not use `.item()`, `.cpu()`, `.tolist()`, or device-to-host tensor copies. The shared H3 runtime is switched into `forbid_device_sync` mode when Chipmunk is installed; sampler evaluations are counted from the explicit sampler boundary instead of reading sigma values from CUDA.
...
The first measurement showed the strongest concentration in the early transformer blocks, so the next test deliberately limits approximation to that region:
...
Layers 15-49 remain dense. This is intentionally conservative: it tests a real approximate video while keeping the persistent GPU cache substantially smaller than an all-layer experiment.
...
Before allocating reference-delta state, the executor estimates cache usage from dynamic-token count, enabled layer range, hidden width, and selected feature width. It raises if the estimate exceeds `cache_budget_gb`.
...
This remains a research implementation using existing Comfy/Comfy-Kitchen ConvRot INT8 operations. Selected `fc1` rows and `fc2` columns are gathered before calling the existing kernels; there are not yet bespoke fused sparse kernels. Therefore a successful quality test does not imply a wall-clock speedup yet.

The next optimization stage, if this CUDA-only quality run is acceptable, is fused group-indexed ConvRot execution that avoids `index_select(...).contiguous()` weight gathers and fuses the old/new selected `fc2` contribution update.
