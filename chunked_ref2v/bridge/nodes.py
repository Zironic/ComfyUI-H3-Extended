...
def format_report(plan, results, notes, *, seed, arm_ids):
    lines = [
...
def _seam_playback(context, record, window=16):
    """Left tail + generated + right head, so a seam is visible in one clip."""
    parts = [context.left["pixels"][-window:], record["pixels"],
...
class MiniMaxH3BridgeExperiment(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.String.Input("preview_arm", default="", multiline=False,
                                tooltip="Which arm the seam playback shows. Blank "
                                        "picks the last completed arm."),
            ...
            outputs=[
                io.Image.Output(display_name="seam_playback"),
    ...
    def execute(cls, model, clip, video_vae, audio_vae, source_video, base_prompt,
        ...
        if selected in results and results[selected].get("status") == "completed":
            playback = _seam_playback(context, results[selected])
        else:
            playback = context.ground_truth.to("cpu", torch.float32)

        return io.NodeOutput(playback, report, manifest_path)
